...
let dbExtrato = new Db('nubank/extrato');
let dbImport = new Db('nubank/faturaImportada');
let dbAuth = new Db('nubank/credenciais');
let python;

ipcMain.on('salvarExtratoNubankOfx', async (event, item) => {
  // try {
    let arquivo = item[0];
    const ofxString = await fs.promises.readFile(arquivo.caminho, 'utf-8');

    parseOFX(ofxString).then(ofxData => {
      const response = ofxData.OFX.BANKMSGSRSV1.STMTTRNRS.STMTRS;
      const transaction = response.BANKTRANLIST;
      // dbExtrato.cadastrar
      // console.log(ofxData);
      ...
      event.reply('exibeExtratoNubankOfx', response);
    });
